...
{
	IPCCommandResult Reply = IWII_IPC_HLE_Device::GetNoReply();
	...
	case USBV0_IOCTL_DEVINSERTHOOK:
		{
			...
			// It is inserted
			Reply = IWII_IPC_HLE_Device::GetDefaultReply();
		}
		break;
...
IPCCommandResult CWII_IPC_HLE_Device_usb_ven::IOCtl(u32 CommandAddress)
{
	...
	case USBV5_IOCTL_GETDEVICECHANGE:
		...
		if (firstcall)
			Reply = IWII_IPC_HLE_Device::GetDefaultReply(), firstcall = false;
		// num devices
